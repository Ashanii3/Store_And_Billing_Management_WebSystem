...
if(isset($_GET['payments?deleting'])){
    $del_ID=$_GET['payments?deleting'];

    if(isset($_POST['delete'])){
            $delete="DELETE from payments where payment_id=$del_ID";
            $db = mysqli_connect("localhost", "root", "", "project");
            $result=mysqli_query($db, $delete);
            if($result){
                echo "<script>alert('Payment details deleted Successfully') </script>";
                echo "<script>window.open('adminProfile.php?payments', '_self') </script>";
            }
    }
...
<body>
    <h2 class="text-center mt-5 mb-3">Want to delete payment Id <span style="color: red;"><?php echo $del_ID;?> </span> ?</h2>
